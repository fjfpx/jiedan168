...
<body>
	<div class="wrap js-check-wrap">
		<ul class="nav nav-tabs">
            <li><a href="<?php echo U('AdminLoan/toverify');?>">待审核列表</a></li>
            <li class="active"><a href="#">借款信息</a></li>
            <li><a href="<?php echo U('AdminLoan/verify_credit',array('id'=>$verify['id']));?>">信用信息</a></li>
		</ul>
			<div class="row-fluid">
            ...
            <div style="padding-left:400px">
                <textarea name="refused_msg" id="refused_msg"></textarea> *拒绝理由
            </div>
			<div class="form-actions" style="padding-left:400px;">
				<a class="btn btn-primary" style="width:75px;" href="<?php echo U('AdminLoan/verify_status_p',array('id'=>$verify['id'],'status'=>1));?>">通过</a>&emsp;&emsp;
				<a class="btn" id="jujue" href="javascript:;" style="width:75px">拒绝</a>
			</div>
	</div>
...
      }});
	...
	<script type="text/javascript">
        var verify = <?php echo ($verify['id']); ?>;
		$(function() {
            $('#jujue').click(function(){
               //var val = $('#refused_msg').value;
               var val = document.getElementById("refused_msg").value;
               var val = val.replace(/(^\s*)|(\s*$)/g, ""); 
               if(val.length <1){
                 alert('拒绝原因不能为空');
               }else{
                var href = "/index.php?g=&m=AdminLoan&a=verify_status_p&id="+verify+"&status=2&refused_msg="+val;
                location.href= href;
               }
            });
