...
class LoanMallController extends AdminbaseController
{
    ...
    {
        $id       = I("get.id");
        $loanMall = $this->loanMall->where("id=$id")->find();
        $this->assign('loanmall', $loanMall);
        $this->display();
    }
        ...
        if (IS_POST) {
            ...
            if ($loanMall) {
                if ($_FILES['logo']) {
                    ...
                    if ($_FILES['logo']) {
                        $fullpath = "http://cms.jiedan168.com/uploads/{$info['logo']['savepath']}/{$info['logo']["savename"]}";
                    }
                    $loanMall['logo'] = $fullpath;
                }
                if ($this->loanMall->save($loanMall)) {
                    $this->success("保存成功！");
                } else {
                    $this->error("保存失败！");
                }
