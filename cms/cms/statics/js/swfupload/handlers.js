...
function att_show(serverData,file)
{
	...
	var img = "";
	if(ext == 1) {
	    img = '<div class="get selected" id="aid-'+num+'"><a class="del" href="javascript:;">删除</a> <img onclick="att_cancel(this,'+num+',\'upload\')" width="87" height="98" src="'+src+'" data-id="'+num+'" data-path="'+src+'" alt="上传完成" title="'+filename+'"><input type="text" class="J_file_desc" name="flashatt['+num+'][desc]" placeholder="请输入描述" value="'+filename+'" style="width:68px">&emsp;&emsp;<a href="javascript:;" onclick="att_delete('+file.id+','+num+',\'upload\')">移除</a></div>';
	} else {
    ...
    uploadnums++;
    var span = '';
    if(uploadnums == window.uploadcounts){
        span = "<span style='color:green;'>上传完成!</span>";
        $('#fsmsg').html(span);
        uploadnums = 0;
        window.uploadcounts = 0;
    }else{
        span = "<span style='color:red;'>上传中...</span>";
        $('#fsmsg').html(span);
    }
}
...
//上传文件列表选中删除
function att_delete(file,aid,source){
    if(source=='upload'){
        $("#"+file.id).remove();
        //$('#att-status-del').append('|'+aid);
        var selected = $("#fsUploadProgress .selected");
		$("#aid-"+aid).removeClass("selected");
		selected = $("#fsUploadProgress .selected");
		//取得被选中图片集合
		var imgstr = $("#att-status").html();
		//计算被选中的附件长度
...
{
	try {
        uploadcounts = numFilesSelected;
        //升序排列
        var obj = document.getElementById("fsUploadProgress");
