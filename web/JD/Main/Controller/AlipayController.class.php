...
class AlipayController extends BaseController {
    ...
    public function taskcreate(){
        ...
        file_put_contents(C('PATH_LOG_MOXIE').'alipay_create',json_encode($rst).PHP_EOL, FILE_APPEND);
    ...
    public function tasklogin(){
        header("HTTP/1.1 201 Created");
        $rst = json_decode(file_get_contents('php://input'),true);
        file_put_contents(C('PATH_LOG_MOXIE').'alipay_login',json_encode($rst).PHP_EOL, FILE_APPEND);
        if(!empty($rst) && is_array($rst)){
            $data = array(
                'user_id' => $rst['user_id'],
            );
                ...
                if( M("Moxie")->where(array('uid'=>$data['user_id'],'task_id'=>$rst['task_id']))->find() ){
                    if(M("Moxie")->where(array('uid'=>$data['user_id'],'task_id'=>$rst['task_id']))->save($arr) !== false){
                        if($arr['login_status']==1){
                            M("MemberAlipay")->where(array('uid'=>$data['user_id']))->save(array('status'=>3));
                        }
                        exit("success");
                    }
                }
    ...
    public function taskbill(){
        header("HTTP/1.1 201 Created");
        $rst = json_decode(file_get_contents('php://input'),true);
        file_put_contents(C('PATH_LOG_MOXIE').'alipay_bill',json_encode($rst).PHP_EOL, FILE_APPEND);
        if(!empty($rst) && is_array($rst)){
            $data = array(
                'user_id' => $rst['user_id'],
            );
    ...
    public function taskfail(){
        header("HTTP/1.1 201 Created");
        $rst = json_decode(file_get_contents('php://input'),true);
        file_put_contents(C('PATH_LOG_MOXIE').'alipay_fail',json_encode($rst).PHP_EOL, FILE_APPEND);
        if(!empty($rst) && is_array($rst)){
            $data = array(
                'user_id' => $rst['user_id'],
            );
    ...
    public function taskreport(){
        header("HTTP/1.1 201 Created");
        $rst = json_decode(file_get_contents('php://input'),true);
        file_put_contents(C('PATH_LOG_MOXIE').'alipay_report',json_encode($rst).PHP_EOL, FILE_APPEND);
        if(!empty($rst) && is_array($rst)){
            $data = array(
                'user_id' => $rst['user_id'],
            );
