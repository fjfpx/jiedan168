	...
	function add_post(){
		if(IS_POST){
			if ($this->slide_model->create()) {
                ...
                $_POST['slide_pic']=preg_replace("/^\./", "", $_POST['slide_pic']);

				if(!empty($_POST['slide_pic'])){
					$thumb =  '.'.$_POST['slide_pic'];
					$image = new \Think\ImageResize($thumb, $thumb, 80);
					$width = $image->getWidth();
					if($width >= 690){
						$image->resizeToWidth(690);
						$image->save();
					}
				}
