...
<body>
	<div class="wrap js-check-wrap">
		...
		<form class="well form-search" method="post" action="">
            ...
            添加人: <input type="text" name="user_login" class="input" value="<?php echo ($formget["user_login"]); ?>" style="width:80px;"/>&emsp;
			...
			<table class="table table-hover table-bordered table-list">
				<thead>
                    ...
                    <?php if(is_array($item['item'])): foreach($item['item'] as $key=>$it): ?><tr>
                        ...
                        <td><?php echo ($it["borrowing_actual_money"]); ?></td>
                        <td><?php echo ($it["user_login"]); ?></td>
                        <td><?php echo ($it["verify_user"]); ?></td>
                        <td><?php echo ($it["verify_time"]); ?></td>
                        <?php $status = array('待打款','待还款','已到期','已还款','已逾期','逾期还款'); ?>
	...
	<script>
		function refersh_window() {
			...
			if (refersh_time == 1) {
				window.location = "<?php echo U('AdminLoan/topay',$formget);?>";
			}
		}
