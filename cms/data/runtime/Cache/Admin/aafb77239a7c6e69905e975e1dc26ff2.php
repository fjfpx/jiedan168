				...
				<?php if(is_array($users)): foreach($users as $key=>$vo): ?><tr>
					...
					<td>
						...
						<?php else: ?>
							...
							<?php if($vo['user_status'] == 1): ?><a href="<?php echo U('user/ban',array('id'=>$vo['id']));?>" class="js-ajax-dialog-btn" data-msg="确认锁定吗?">锁定</a>
							<?php else: ?>
								<a href="<?php echo U('user/cancelban',array('id'=>$vo['id']));?>" class="js-ajax-dialog-btn" data-msg="确认解锁码?">解锁</a><?php endif; endif; ?>
					</td>
