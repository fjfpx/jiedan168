...
class RbacController extends AdminbaseController {
    ...
    public function roleadd_post() {
    	if (IS_POST) {
    		if ($this->role_model->create()) {
                $ar = $this->role_model->add();
    			if ($ar!==false) {
                    $msg = '添加角色成功,角色ID为{'.$ar.'},角色名为{'.$_POST['name'].'}';
                    $this->logs($msg,$ar,2);
    ...
    public function roledelete() {
        ...
        if($count){
            ...
            $rn = M("Role")->where(array('id'=>$id))->getField('name');
        	$status = $this->role_model->delete($id);
        	if ($status!==false) {
                $msg = '删除角色成功,角色ID为{'.$id.'},角色名为{'.$rn.'}';
                $this->logs($msg,$id,2);
    	...
    	if (IS_POST) {
    		...
    		if (is_array($_POST['menuid']) && count($_POST['menuid'])>0) {
    			...
    			foreach ($_POST['menuid'] as $menuid) {
    				$menu=$menu_model->where(array("id"=>$menuid))->field("app,model,action,name")->find();
    				if($menu){
    					$app=$menu['app'];
    					$model=$menu['model'];
    					$action=$menu['action'];
